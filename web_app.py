...
DEFAULT_GAIN = [
    (30, -35.0), (60, -24.5), (90, -12.5), (500, -1.0),
...
def fspl_db(distance_m, freq_mhz):
    if distance_m <= 0:
        return 0.0
    return 20.0 * math.log10(4.0 * math.pi * distance_m * freq_mhz * 1e6 / C)
...
def resample_path(points, n):
    ...
    for k in range(n):
        ...
        a = points[seg - 1]
        b = points[seg]
        length = cumulative[seg] - cumulative[seg - 1]
...
def make_map(draw_mode, center=(31.8, 35.0), zoom=7):
    ...
    if draw_mode == "path":
        options = {"polyline": True, "polygon": False, "rectangle": False,
                   "circle": False, "marker": False, "circlemarker": False}
    elif draw_mode == "aoi":
        options = {"polyline": False, "polygon": True, "rectangle": True,
                   "circle": False, "marker": False, "circlemarker": False}
    else:
        options = {"polyline": False, "polygon": False, "rectangle": False,
                   "circle": False, "marker": False, "circlemarker": False}
    Draw(export=False, position="topleft", draw_options=options,
...
def add_saved_geometry(m):
    if st.session_state.get("path"):
        folium.PolyLine(st.session_state.path, color="red", weight=5,
    ...
    if st.session_state.get("aoi"):
        folium.Polygon(st.session_state.aoi, color="blue", weight=3,
                       fill=True, fill_opacity=0.12, tooltip="AOI").add_to(m)


@st.cache_resource(show_spinner=False)
def get_srtm():
    return srtm.get_data(srtm1=False, srtm3=True)
...
def terrain_elevation(provider, lat, lon, cache):
    ...
    if key in cache:
        return cache[key]
    try:
        value = provider.get_elevation(lat, lon, approximate=True)
...
def los_clear(provider, aircraft_lat, aircraft_lon, aircraft_alt_m,
              ground_lat, ground_lon, ground_elev_m, cache, samples=10):
    for k in range(1, samples):
        t = k / samples
        lat = aircraft_lat + (ground_lat - aircraft_lat) * t
        lon = aircraft_lon + (ground_lon - aircraft_lon) * t
        terrain = terrain_elevation(provider, lat, lon, cache)
        ray_height = aircraft_alt_m + (ground_elev_m - aircraft_alt_m) * t
        if terrain > ray_height + 5.0:
            return False
    return True
...
# These are the actual selection controls. Each button enables the matching map tool.
...
if st.session_state.draw_mode == "path":
    st.info("Flight Path mode: use the **line button in the map toolbar** to draw the aircraft route.")
else:
    st.info("AOI mode: use **rectangle or polygon in the map toolbar** to draw the analysis area.")

center = [31.8, 35.0]
if st.session_state.path:
    center = st.session_state.path[len(st.session_state.path) // 2]
elif st.session_state.aoi:
    center = [sum(p[0] for p in st.session_state.aoi) / len(st.session_state.aoi),
              sum(p[1] for p in st.session_state.aoi) / len(st.session_state.aoi)]
...
if new_drawings:
    feature = new_drawings[-1]
    geometry = feature.get("geometry", {})
    coords = geometry.get("coordinates", [])
    if st.session_state.draw_mode == "path" and geometry.get("type") == "LineString":
        st.session_state.path = [[p[1], p[0]] for p in coords]
        st.session_state.analysis = None
    elif st.session_state.draw_mode == "aoi" and geometry.get("type") == "Polygon" and coords:
        st.session_state.aoi = [[p[1], p[0]] for p in coords[0]]
        st.session_state.analysis = None

c1, c2 = st.columns(2)
with c1:
    st.write("**Flight Path:**", "✅ Selected" if st.session_state.path else "❌ Not selected")
with c2:
    st.write("**AOI:**", "✅ Selected" if st.session_state.aoi else "❌ Not selected")

run = st.button("🚀 Run DTM + SNR Analysis", type="primary", use_container_width=True,
                disabled=not (st.session_state.path and st.session_state.aoi))

if run:
    path = list(st.session_state.path)
    aoi = list(st.session_state.aoi)
    if min_freq > max_freq:
        st.error("Max Frequency must be greater than or equal to Min Frequency.")
        st.stop()
    if snr_max <= snr_min:
        st.error("SNR Display Max must be greater than SNR Display Min.")
        st.stop()

    freqs = [min_freq] if int(steps) == 1 else np.linspace(min_freq, max_freq, int(steps)).tolist()
    sampled_path = resample_path(path, int(path_samples))
    aircraft_alt_m = float(altitude_kft) * 304.8
    ...
    log_line(log_box, lines, f"AOI: {len(aoi)} vertices.")
    log_line(log_box, lines, f"Frequencies: {len(freqs)}.")
    log_line(log_box, lines, "Loading SRTM/DTM terrain data...")

    try:
        dtm = get_srtm()
    except Exception as exc:
        st.error(f"DTM/SRTM could not be loaded: {exc}")
        st.stop()

    log_line(log_box, lines, "SRTM/DTM provider ready.")
    ...
    n = int(grid_size)
    lat_vals = np.linspace(max_lat, min_lat, n)
    lon_vals = np.linspace(min_lon, max_lon, n)

    cells = [(iy, ix, float(lat), float(lon))
             for iy, lat in enumerate(lat_vals)
             for ix, lon in enumerate(lon_vals)
             if point_in_polygon(float(lat), float(lon), aoi)]
    total_cells = max(1, len(cells))
    log_line(log_box, lines, f"AOI raster contains {len(cells)} cells.")
    log_line(log_box, lines, "Reading DTM elevation for AOI cells...")
    ...
    for idx, (_, _, lat, lon) in enumerate(cells, 1):
        ...
        if idx == 1 or idx == len(cells) or idx % max(1, len(cells) // 10) == 0:
            progress.progress(0.10 * idx / total_cells, text=f"Loading DTM: {idx}/{len(cells)}")

    log_line(log_box, lines, "DTM loaded. Calculating SNR from aircraft-to-ground distance.")
    if los_enabled:
        log_line(log_box, lines, "DTM terrain masking is enabled.")

    snr_grid = np.full((n, n), np.nan, dtype=np.float32)
    best_meta = {}
    completed = 0

    for iy, ix, glat, glon in cells:
        gelev = ground_elevation[(round(glat, 5), round(glon, 5))]
        best_snr = -999.0
        best_aircraft = None
        best_freq = None
        blocked = False

        for ai, (alat, alon) in enumerate(sampled_path, 1):
            horizontal = haversine_m(alat, alon, glat, glon)
            d3 = math.sqrt(horizontal ** 2 + (aircraft_alt_m - gelev) ** 2)
            for freq in freqs:
                pr = tx_power + interp_gain(float(freq)) - fspl_db(d3, float(freq)) - pol_loss - sys_loss
                snr = pr - noise_floor
                visible = True
                if los_enabled and horizontal > 1.0:
                    visible = los_clear(dtm, alat, alon, aircraft_alt_m,
                                        glat, glon, gelev, elevation_cache, int(los_samples))
                if not visible:
                    snr = -150.0
                    blocked = True
                if snr > best_snr:
        ...
        snr_grid[iy, ix] = best_snr
        best_meta[(iy, ix)] = (best_aircraft, best_freq, gelev, blocked)
        completed += 1

        if completed == 1 or completed == len(cells) or completed % max(1, len(cells) // 20) == 0:
            elapsed = time.time() - started
            rate = completed / elapsed if elapsed > 0 else 0.0
            remaining = (len(cells) - completed) / rate if rate else 0.0
            pct = completed / total_cells
            progress.progress(0.10 + 0.88 * pct, text=f"Calculating SNR: {completed}/{len(cells)} cells")
            eta_box.info(f"Progress: {pct*100:.1f}%  |  Elapsed: {elapsed:.1f}s  |  Estimated remaining: {remaining:.1f}s")
            log_line(log_box, lines, f"SNR: {completed}/{len(cells)} cells | ETA {remaining:.1f}s")

    elapsed = time.time() - started
    progress.progress(1.0, text="Analysis complete")
    eta_box.success(f"Completed in {elapsed:.1f}s")
    log_line(log_box, lines, f"Analysis complete in {elapsed:.1f}s.")
    log_line(log_box, lines, "SNR includes the real 3D distance from every sampled aircraft position to every AOI cell.")
    if los_enabled:
        log_line(log_box, lines, "DTM elevation and terrain LOS masking were applied.")
...
if st.session_state.analysis:
    ...
    result_map = folium.Map(
        ...
        zoom_start=9, control_scale=True, tiles="OpenStreetMap"
    )
    add_saved_geometry(result_map)
    ...
    if valid.size:
        ...
        c1.metric("Aircraft samples", len(analysis["path"]))
        c2.metric("Max SNR", f"{float(np.max(valid)):.1f} dB")
        c3.metric("Min SNR", f"{float(np.min(valid)):.1f} dB")
    st.caption("SNR is calculated per AOI cell from aircraft-to-ground 3D distance. DTM/SRTM supplies ground elevation and the terrain profile used for LOS masking.")
else:
