...
def fspl_db(distance_m, freq_mhz):
    return 0.0 if distance_m <= 0 else 20*math.log10(4*math.pi*distance_m*freq_mhz*1e6/C)
...
def resample_path(points, n):
    ...
    for k in range(n):
        ...
        t=(target-cumulative[seg-1])/length if length else 0
...
def terrain_elevation(provider, lat, lon, cache):
    ...
    try:
        v=provider.get_elevation(lat,lon,approximate=True)
...
def base_map(center, zoom):
    return folium.Map(location=list(center),zoom_start=zoom,control_scale=True,tiles="OpenStreetMap")
...
def add_auto_draw(m, mode):
    ...
    opts={
        ...
        "polygon": False,
        "rectangle": {"shapeOptions":{"color":"#3388ff","weight":3,"fillOpacity":0.12}} if mode=="aoi" else False,
        "circle": False,"marker": False,"circlemarker": False
    }
    draw=Draw(export=False,position="topleft",draw_options=opts,edit_options=False)
    draw.add_to(m)
    handler="polyline" if mode=="path" else "rectangle"
    map_name=m.get_name()
    js=f"""
    ...
    (function() {{
      function startDrawing() {{
        try {{
          var map = {map_name};
          if (map && map._toolbars && map._toolbars.draw && map._toolbars.draw._modes) {{
            var h = map._toolbars.draw._modes['{handler}'];
            if (h && h.handler) h.handler.enable();
          }}
...
def add_geometry(m):
    ...
    if st.session_state.aoi:
        folium.Polygon(st.session_state.aoi,color="blue",weight=3,fill=True,fill_opacity=.10,tooltip="AOI").add_to(m)


def add_heat_cells(m, grid, details, bounds, vmin, vmax):
    rows,cols=grid.shape
    minlat,minlon=bounds[0]; maxlat,maxlon=bounds[1]
    dlat=(maxlat-minlat)/rows; dlon=(maxlon-minlon)/cols
    for iy in range(rows):
        for ix in range(cols):
            detail=details[iy][ix]
            if detail is None: continue
            north=maxlat-iy*dlat; south=maxlat-(iy+1)*dlat
            west=minlon+ix*dlon; east=minlon+(ix+1)*dlon
            center_lat=(south+north)/2; center_lon=(west+east)/2
            if detail["blocked"]:
                fill="#707070"
                tip="LOS BLOCKED"
                popup_html=(f"<div style='min-width:230px'><b>LOS BLOCKED</b><br>"
                            f"Lat: {center_lat:.5f}<br>Lon: {center_lon:.5f}<hr>"
                            f"No valid RF path: terrain blocks the line of sight.<br>"
                            f"DTM elevation: {detail['ground_elev']:.1f} m</div>")
            else:
                v=detail["snr"]
                r,g,b=color_rgb(v,vmin,vmax); fill=f"rgb({r},{g},{b})"
                tip=f"{v:.1f} dB"
                popup_html=(f"<div style='min-width:260px'><div style='font-size:20px;font-weight:700'>{v:.1f} dB</div>"
                            f"<hr><b>Calculation</b><br>"
                            f"Frequency: {detail['freq']:.3f} MHz<br>"
                            f"Distance: {detail['distance_km']:.3f} km<br>"
                            f"Tx Power: {detail['tx_power']:.1f} dBm<br>"
                            f"Antenna Gain: {detail['gain']:.1f} dB<br>"
                            f"FSPL: {detail['fspl']:.1f} dB<br>"
                            f"Polarization Loss: {detail['pol_loss']:.1f} dB<br>"
                            f"System Loss: {detail['sys_loss']:.1f} dB<br>"
...
with st.sidebar:
    ...
    if st.button("🗑️  נקה בחירות",use_container_width=True):
        st.session_state.path=None; st.session_state.aoi=None
        st.session_state.path_confirmed=False; st.session_state.aoi_confirmed=False
        st.session_state.analysis=None; st.session_state.mode=None
        st.rerun()
    ...
    st.subheader("RF / DTM")
    path_samples=st.number_input("Flight Path Samples",2,200,30,1)
    grid_size=st.number_input("AOI Grid Resolution",15,60,35,5)
    los_enabled=st.checkbox("DTM Line-of-Sight Masking",True)
    los_samples=st.number_input("DTM LOS Samples",10,150,40,5)
    ...
    if st.button("🚀  Run DTM + SNR Analysis",type="primary",use_container_width=True,disabled=not ready):
...
if st.session_state.analysis:
    bounds=st.session_state.analysis["bounds"]
    center=[(bounds[0][0]+bounds[1][0])/2,(bounds[0][1]+bounds[1][1])/2]
    m=base_map(center,9)
    add_heat_cells(m,st.session_state.analysis["grid"],st.session_state.analysis["details"],bounds,scale_min,scale_max)
    add_geometry(m)
    add_legend(m,scale_min,scale_max)
else:
...
if not st.session_state.analysis:
    ...
    if drawings:
        ...
        changed=False
        if st.session_state.mode=="path" and geo.get("type")=="LineString" and len(coords)>=2:
            st.session_state.path=[[p[1],p[0]] for p in coords]; st.session_state.path_confirmed=False; changed=True
        elif st.session_state.mode=="aoi" and geo.get("type")=="Polygon" and coords:
...
if st.session_state.run_requested and ready:
    ...
    if min_freq>max_freq:
        st.error("Max Frequency must be >= Min Frequency."); st.stop()
    if scale_max<=scale_min:
        st.error("Color Bar Max must be greater than Min."); st.stop()

    freqs=[min_freq] if int(steps)==1 else np.linspace(min_freq,max_freq,int(steps)).tolist()
    sampled_path=resample_path(st.session_state.path,int(path_samples))
    aircraft_alt_m=float(altitude_kft)*304.8
    noise_floor=-174.0+10*math.log10(NOISE_BW_MHZ*1e6)+float(noise_figure)

    progress=st.progress(0.0,text="Loading DTM...")
    started=time.time()
    try:
        dtm=get_srtm()
    except Exception as e:
        st.error(f"DTM/SRTM could not be loaded: {e}"); st.stop()
    ...
    n=int(grid_size)
    latvals=np.linspace(maxlat,minlat,n); lonvals=np.linspace(minlon,maxlon,n)
    ...
    for i,(_,_,lat,lon) in enumerate(cells,1):
        ...
        if i==1 or i==len(cells) or i%max(1,len(cells)//10)==0:
            progress.progress(.12*i/max(1,len(cells)),text=f"DTM {i}/{len(cells)}")

    grid=np.full((n,n),np.nan,dtype=np.float32)
    details=[[None for _ in range(n)] for _ in range(n)]
    blocked=0

    for i,(iy,ix,glat,glon) in enumerate(cells,1):
        gelev=elev[(round(glat,5),round(glon,5))]
        best=-math.inf; best_detail=None
        for alat,alon in sampled_path:
            horiz=haversine_m(alat,alon,glat,glon)
            d3=math.sqrt(horiz*horiz+(aircraft_alt_m-gelev)**2)
            visible=los_clear(dtm,alat,alon,aircraft_alt_m,glat,glon,gelev,cache,int(los_samples)) if los_enabled else True
            if not visible: continue
            for f in freqs:
                gain=interp_gain(float(f)); fspl=fspl_db(d3,float(f))
                snr=tx_power+gain-fspl-pol_loss-sys_loss-noise_floor
                if snr>best:
                    best=snr
                    best_detail={"snr":snr,"freq":float(f),"distance_km":d3/1000.0,"tx_power":tx_power,"gain":gain,"fspl":fspl,"pol_loss":pol_loss,"sys_loss":sys_loss,"noise_floor":noise_floor,"ground_elev":gelev,"blocked":False}
        if best_detail is None:
            blocked+=1
            details[iy][ix]={"blocked":True,"ground_elev":gelev}
        else:
            grid[iy,ix]=best
            details[iy][ix]=best_detail
        if i==1 or i==len(cells) or i%max(1,len(cells)//20)==0:
            progress.progress(.12+.88*i/max(1,len(cells)),text=f"DTM + SNR {i}/{len(cells)}")

    elapsed=time.time()-started
    progress.progress(1.0,text="Analysis complete")
    st.session_state.analysis={"grid":grid,"details":details,"bounds":[[minlat,minlon],[maxlat,maxlon]],"path":sampled_path,"blocked":blocked,"elapsed":elapsed}
    st.rerun()
